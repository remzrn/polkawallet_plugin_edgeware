  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      if (!(<any>window).api) {
        ...
        registry.setMetadata(new Metadata(registry, metaDataMap["edgeware"]));
      } else {
...
async function signBytesAsExtension(password: string, json: any) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      resolve({
        signature: u8aToHex(keyPair.sign(wrapBytes(isDataHex ? hexToU8a(json["data"]) : stringToU8a(json["data"])))),
      });
    } catch (err) {
      resolve({ error: err.message });
    }
